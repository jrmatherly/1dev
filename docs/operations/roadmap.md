...
|------|------|---------------|
| 2026-04-10 | Self-hosted 1code-api backend service — Phase 1 endpoints (health, changelog, plan, profile) verified end-to-end in Docker against Postgres 18, Dockerfile, container-build.yml workflow (multi-arch + Cosign + SLSA), 1code-update-server K8s manifests deleted, 5 service test files (75 total tests). 49/51 tasks — 2 remaining are GHCR tag-push verification | `implement-1code-api` OpenSpec change |
