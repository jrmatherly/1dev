...
|------|------|---------------|
| 2026-04-10 | v0.0.80 patch release — TS baseline 54→45 (Cluster A `DiffStateContextValue` type alignment killed 9 errors), keytar arm64 rebuild postinstall fix (dlopen crash on Apple Silicon fresh installs — extracted inline `node -e` to `scripts/rebuild-native-modules.mjs` which now rebuilds `better-sqlite3,node-pty,keytar` against Electron ABI). Ships unsigned. | `46f49a4`, `30641ce`, `5ac5313`, tag `v0.0.80` |
