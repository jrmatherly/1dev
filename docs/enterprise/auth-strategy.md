...
**See also:** [`llm-routing-patterns.md`](./llm-routing-patterns.md) — the canonical four-pattern matrix (`subscription-direct`, `subscription-litellm`, `byok-direct`, `byok-litellm`) with exact spawn-env recipes, the `MAIN_VITE_ALLOW_DIRECT_ANTHROPIC` gate semantics, and the `x-litellm-customer-id` audit-attribution header. Entra access tokens never flow into Anthropic bearer headers — they're distinct tokens with distinct lifetimes, issuers, and trust domains. The separation is enforced at three layers: the `ProviderMode` type (Entra JWT isn't a field), the `applyEnterpriseAuth()` contract (returns `Promise<void>`), and the project-wide regression guard (`no-entra-in-anthropic-auth-token.test.ts` scans all `.ts`/`.tsx` files under `src/main/` for forbidden bind-then-assign shapes).
...
2. **Create Entra app registration(s)** per §2.2. Add the new SOPS variables (`litellm_envoy_oidc_client_id`, `litellm_envoy_oidc_client_secret`, optionally `litellm_envoy_native_client_id`) — DO NOT reuse `litellm_entra_*` names which are owned by LiteLLM's existing Entra SSO.
   ...
   >
   > The same doc's [**Step 5a — Delegated Graph permissions for the desktop client**](./entra-app-registration-1code-api.md#5a-delegated-graph-permissions-for-the-desktop-client) covers the `User.Read` delegated scope required by `acquireTokenForGraph()` in the Electron app. Admin consent on the desktop app registration is a one-time tenant operation; without it, each user sees a per-account consent prompt on first interactive sign-in.
