...
- **Tier C identifiers** are historical or legal attribution strings required by Apache License 2.0 §4(c)–(d) and SHALL be preserved verbatim: the upstream PR link comment at `src/main/lib/cli.ts:6`, the "forked from 1Code by 21st-dev" attribution line at `README.md:3`, the copyright header in `LICENSE`, the top-level `NOTICE` file, and historical references inside documentation under `docs/`, `.serena/memories/`, `.claude/`, and `openspec/changes/`.
