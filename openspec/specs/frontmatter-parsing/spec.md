...
Define the canonical interface for parsing YAML frontmatter from `.md` files in the main process. The capability ensures all main-process frontmatter parsing flows through a single shim at `src/main/lib/frontmatter.ts` so that:

1. The underlying parser implementation can be swapped without touching consumer call sites
2. Bundled output is empirically free of dynamic-code-evaluation patterns that trip Rollup security warnings
3. Type narrowing of untyped frontmatter data is consistent and sound across consumers
4. A regression guard mechanically prevents drift back to direct parser imports

This mirrors the `credential-storage` pattern where `src/main/lib/credential-store.ts` is the single sanctioned entry point for `safeStorage.*` calls. The shim approach lets the project upgrade or replace the underlying parser package (currently `front-matter@4.0.2`, previously `gray-matter@4.0.3`) without coordinating edits across the consumer files in `src/main/lib/trpc/routers/{commands,plugins,skills,agent-utils}.ts`.
