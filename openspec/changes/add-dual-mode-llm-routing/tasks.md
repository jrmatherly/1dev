...
- [x] 9.1 Redesign `AnthropicAccountsSection` in `src/renderer/components/dialogs/settings-tabs/agents-models-tab.tsx`. Replace the single "Add" button with a wizard dialog.
- [x] 9.2 Step 1: account-type chooser — "Existing Claude Code Subscription" | "Bring Your Own API Key".
- [x] 9.3 Step 2 (routing mode): conditionally rendered when `import.meta.env.MAIN_VITE_ALLOW_DIRECT_ANTHROPIC === "true"`. Otherwise silently lock to `litellm`.
- [x] 9.4 Step 3 (credentials for `claude-subscription`): reuse existing Keychain import flow. For `routingMode="litellm"`, additionally prompt for a LiteLLM virtual key with explanatory copy.
- [x] 9.5 Step 3 (credentials for `byok`): text input for key. Validate: `direct` mode requires match on `/^sk-ant-[a-z0-9-]{30,}/i`; `litellm` mode accepts any non-empty string.
- [x] 9.6 Step 4 (BYOK-LiteLLM only): "Fetch Models" button calling `trpc.litellmModels.listUserModels`. On success, show three dropdowns (Sonnet/Haiku/Opus) pre-filled with regex-best-match from returned ids; on failure show error + three plain text inputs.
- [x] 9.7 Persist via an extended `trpc.anthropicAccounts.add` mutation that accepts the full new shape.

### Subscription-aware model picker access control (appended 2026-04-13)

- [x] 9.8 Extend the existing `trpc.anthropicAccounts.getActive` query in `src/main/lib/trpc/routers/anthropic-accounts.ts:79–95` to include `accountType: anthropicAccounts.accountType` and `routingMode: anthropicAccounts.routingMode` in the `.select({...})` shape (enum columns, not credentials — no `decryptCredential` wrapping). Ensure the legacy-fallback return branch (where `legacyCred` is read in the `// Fallback: if legacy credential exists` block) also returns `accountType: "claude-subscription"` and `routingMode: "direct"` so consumers see a uniform shape.
- [x] 9.9 In `src/renderer/features/agents/main/new-chat-form.tsx` (around the `<AgentModelSelector>` render at line 1966+), add `const { data: activeAccount } = trpc.anthropicAccounts.getActive.useQuery();` following the existing pattern at `src/renderer/components/dialogs/settings-tabs/agents-models-tab.tsx:154`. Compute `const canAddModels = !(activeAccount?.accountType === "claude-subscription" && enterpriseAuthEnabled);` and conditionally pass `onOpenModelsSettings` to `<AgentModelSelector>` only when `canAddModels === true`. The "Add Models" footer button in the picker (`agent-model-selector.tsx:683–693`) renders only when `onOpenModelsSettings` is truthy, so withholding it on Claude-subscription + enterprise-auth sessions hides the BYOK-provisioning entry point from end users on managed subscriptions while leaving all other picker behavior (Codex, Ollama, thinking toggles) intact.
- [x] 9.10 Add regression guard `tests/regression/subscription-lock-model-picker.test.ts` that (a) scans `new-chat-form.tsx` for the `canAddModels` gate expression, and (b) asserts the gate references both `accountType === "claude-subscription"` and `enterpriseAuthEnabled`. Follow the grep-based structural pattern used by `tests/regression/no-entra-in-anthropic-auth-token.test.ts`.
- [ ] 9.11 Manual smoke: with `MAIN_VITE_DEV_BYPASS_AUTH=true` and an active `claude-subscription` account, verify the model-picker dropdown no longer shows the "Add Models" footer. Toggle the active account to `byok` (via the wizard once §9.1–9.7 land) and verify "Add Models" reappears.
