...
Provide LiteLLM user, team, and API key lifecycle management inside the `services/1code-api` Fastify service, driven by Entra security group membership. This capability covers the provisioning state machine, team assignment from Entra groups, API key generation and lifecycle (list/rotate/revoke), the deprovisioning cron, and the rotation cron. It enables 1Code desktop app users to self-provision LiteLLM access without going through an external portal, and unblocks the eventual decommission of the Apollos portal service in the `talos-ai-cluster`.
