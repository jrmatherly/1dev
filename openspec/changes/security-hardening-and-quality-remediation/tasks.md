...
- [x] 8.7 Address 96 `as any` casts in `src/` — systematic sweep per-directory, prioritizing security-sensitive files first *(PARTIAL — main-process sweep complete: 18 → 2 casts (89% reduction); renderer remainder 77 casts deferred as follow-up per spec's "systematic sweep per-directory" guidance. Main-process wins: new `src/main/global.d.ts` augmentation eliminates all `(global as any).__xyz` casts in windows/main.ts, auto-updater.ts, index.ts; `McpServerConfig` property narrowing in claude.ts; indexed-record narrowing for Codex JSON-RPC deserialization sites.)*
