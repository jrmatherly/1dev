...
A **new, distinct** confidential-client Entra app registration for server-side Microsoft Graph calls (`onecode_api_graph_client_id` + `onecode_api_graph_client_secret` in `cluster.yaml`). The existing public-client app reg (`onecode_api_entra_client_id` in `cluster.yaml`; literal GUID scrubbed from this archived record 2026-04-11) used by MSAL Node in the desktop app stays unchanged. The new confidential client needs `GroupMember.Read.All` application permission with admin consent granted.
