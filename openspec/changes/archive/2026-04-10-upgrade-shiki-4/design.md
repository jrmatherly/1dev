...
**[Risk] A future session edits `upgrade-vite-8-build-stack` without knowing this change removed its §10 Shiki work, causing conflict** → **Mitigation:** Phase 5 of `tasks.md` explicitly removes §10 from `upgrade-vite-8-build-stack` `proposal.md`, `design.md`, and `tasks.md` as part of this change's merge cleanup. The edits are scoped tasks, not drive-bys. Tasks use section-header citations (not line numbers) so they survive file drift between proposal authoring and execution.

**[Risk] Shiki 4 may have tightened the `loadTheme` input schema for tokenColors-bearing themes** → **Mitigation:** `src/renderer/lib/themes/shiki-theme-loader.ts` line 131 (`loadFullTheme`) constructs a literal theme object with `{ name, type, colors, tokenColors }` and calls `highlighter.loadTheme(shikiTheme)`. If Shiki 4 renames or adds required fields to this schema, the `try`/`catch` at line 139 swallows the failure and logs `console.error`. Phase 3 of `tasks.md` requires exercising at least one theme with populated `tokenColors` (a `BUILTIN_THEMES` Cursor theme) during runtime verification, AND scanning dev-console output for `Failed to load full theme` error lines.
