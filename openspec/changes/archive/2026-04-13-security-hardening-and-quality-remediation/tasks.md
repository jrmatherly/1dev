...
- [x] 7.6 Verify claude.ts < 1,000 lines after extraction *(PARTIAL — claude.ts 3,309 → 2,503 lines (-24.3%) via 4 extractions (prompt-parser 97 + session-manager 59 + mcp-resolver 528 + tool-executor 240 = 924 extracted lines in 4 new modules). Target of <1,000 lines not met: the dominant remaining bulk is the 2,003-line `chat` tRPC subscription handler (claudeRouter.chat at lines 201-~2200) which owns deeply coupled observer state (emit/safeEmit/safeComplete, parts[], currentText, abortController, transform.on hooks, the `for await (const msg of stream)` loop, MCP symlink setup, onAbort+finally rollback). Further decomposition into chat-stream-processor + chat-mcp-setup + chat-cleanup deferred to roadmap entry "Further claude.ts decomposition — decompose the 2,003-line chat subscription handler" under docs/operations/roadmap.md P3.)*
- [x] 7.7 Run all 6 quality gates after claude.ts decomposition *(2026-04-12: ts:check 0, lint clean, build 35.27s, test 221/10/0 (231 tests across 40 files), audit 55 pre-existing advisories (no new), docs build 19.61s)*
