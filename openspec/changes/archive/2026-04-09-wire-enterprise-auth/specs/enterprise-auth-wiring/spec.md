...
`ANTHROPIC_AUTH_TOKEN` and `ANTHROPIC_AUTH_TOKEN_FILE` SHALL be added to `STRIPPED_ENV_KEYS_BASE` so that only `applyEnterpriseAuth()` can set them authoritatively (preventing shell-inherited leaks).
...
A new router at `src/main/lib/trpc/routers/enterprise-auth.ts` SHALL expose `signIn`, `signOut`, `getStatus`, `refreshToken` procedures. All check `enterpriseAuthEnabled` and throw `PRECONDITION_FAILED` if disabled. Router count: 21 → 22.
...
4. ~~`auth-manager.ts` does NOT import from `enterprise-auth` (isolation boundary)~~ **REMOVED** — wiring now permitted
