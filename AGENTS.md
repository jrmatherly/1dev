...
- **Claude Code rules (auto-loaded)**: [`.claude/rules/`](.claude/rules/) — global rules (scratchpad) + path-scoped rules (auth-env-vars, credential-storage, database, openspec, testing, tscheck-baseline, upstream-boundary).
...
- IPC: tRPC via `trpc-electron` (21 routers in `createAppRouter`) — see [`docs/architecture/trpc-routers.md`](docs/architecture/trpc-routers.md)
...
- Electron: `~40.8` (upgraded from 39 on 2026-04-09)
