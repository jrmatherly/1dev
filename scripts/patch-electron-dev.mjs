...
if (existsSync(plistPath)) {
  try {
    execSync(
      `/usr/libexec/PlistBuddy -c "Set :CFBundleName 1Code" "${plistPath}"`,
    );
    execSync(
      `/usr/libexec/PlistBuddy -c "Set :CFBundleDisplayName 1Code" "${plistPath}"`,
    );
    console.log("[patch-electron-dev] Updated Info.plist: name -> 1Code");
  } catch (e) {
    console.warn(
      "[patch-electron-dev] Failed to update Info.plist:",
      e.message,
    );
  }
}
