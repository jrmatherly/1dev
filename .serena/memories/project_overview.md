...
- **1code-api service:** Phase 1 implemented (2026-04-10) — `services/1code-api/` with Fastify+tRPC+Drizzle/PostgreSQL. Endpoints: health, changelog, plan, profile. Container: `ghcr.io/jrmatherly/1code-api` via `.github/workflows/container-build.yml`. 17 tests passing. Dead `1code-update-server` K8s manifests deleted.
- **Active OpenSpec changes (2):** `upgrade-vite-8-build-stack` (15/59, Phase A done), `implement-1code-api` (Phase 1 code complete, verification/docs remaining).
