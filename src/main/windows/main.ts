...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    "window:set-frame-preference",
    (_event, useNativeFrame: boolean) => {
      try {
        ...
        writeFileSync(
          settingsPath,
          JSON.stringify({ useNativeFrame }, null, 2),
        );
        return true;
      } catch (error) {
  ...
  ipcMain.handle(
    "window:new",
    (_event, options?: { chatId?: string; subChatId?: string }) => {
  ...
  const validateSender = (event: Electron.IpcMainInvokeEvent): boolean => {
    ...
    try {
      ...
      const trusted = ["apollosai.dev", "localhost", "127.0.0.1"];
  ...
  ipcMain.handle(
    "auth:update-user",
    async (event, updates: { name?: string }) => {
      if (!validateSender(event)) return null;
      try {
