...
Commit message:`;
    ...
    if (!response.ok) {
      console.error(
        "[Ollama] Generate commit message failed:",
        response.status,
      );
      return null;
    }
...
export const chatsRouter = router({
  ...
  rollbackToMessage: publicProcedure
    ...
    .mutation(
      async ({
        ...
        { success: false; error: string } | { success: true; messages: any[] }
      > => {
        const db = getDatabase();
        ...
        if (chat?.worktreePath) {
          const res = await applyRollbackStash(
            chat.worktreePath,
  ...
  getParsedDiff: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const cached = gitCache.getParsedDiff<ParsedDiffResponse>(
        chat.worktreePath,
        diffHash,
      );
      if (cached) {
  ...
  generateCommitMessage: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.filePaths && input.filePaths.length > 0) {
        ...
        files = files.filter((f) => {
          ...
          return (
            ...
            [...selectedPaths].some(
              (sp) => filePath.endsWith(sp) || sp.endsWith(filePath),
            )
          );
        });
        console.log(
      ...
      if (!hasInternet) {
        ...
        if (ollamaMessage) {
          console.log(
            "[generateCommitMessage] Generated via Ollama:",
            ollamaMessage,
          );
          return { message: ollamaMessage };
        }
        console.log(
          "[generateCommitMessage] Ollama failed, using heuristic fallback",
        );
            ...
            if (msg.role === "user") {
              ...
              for (const part of msg.parts || []) {
                if (part.type?.startsWith("tool-") && part.toolName) {
                  toolCalls++;
                  toolUsage[part.toolName] =
                    (toolUsage[part.toolName] || 0) + 1;
                }
              }
