 ...
 *
 * For authenticated users (with subscription): uses apollosai.dev backend
...
async function getUserPlan(): Promise<{
  ...
  if (
    cachedUserPlan &&
    Date.now() - cachedUserPlan.fetchedAt < PLAN_CACHE_TTL_MS
  ) {
...
/**
 * Transcribe audio using apollosai.dev backend (for authenticated users)
 */
  ...
  transcribe: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (hasLocalKey) {
        ...
        console.log(
          `[Voice] Local transcription result: "${text.slice(0, 100)}..."`,
        );
        return { text };
      }
