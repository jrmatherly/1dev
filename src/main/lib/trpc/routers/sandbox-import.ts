...
import {
  importSandboxToWorktree,
  type ExportClaudeSession,
} from "../../git/sandbox-import";
...
async function writeClaudeSession(
  ...
  const rewrittenData = session.data.replace(
    /\/home\/user\/repo/g,
    localProjectPath,
  );
...
export const sandboxImportRouter = router({
  ...
  importSandboxChat: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!chatResponse.ok) {
        throw new Error(
          `Failed to fetch chat data: ${chatResponse.statusText}`,
        );
      }

      const remoteChatData = remoteChatSchema.parse(await chatResponse.json());
      console.log(
        `[OPEN-LOCALLY] Found ${remoteChatData.subChats.length} subchat(s) to import`,
      );
      ...
      if (remoteChatData.subChats.length > 0) {
        ...
        const lastAssistant = [...messagesArray]
          .reverse()
      ...
      console.log(
        `[sandbox-import] Available Claude sessions: ${claudeSessions.length}`,
      );
      ...
      for (const remoteSubChat of remoteChatData.subChats) {
        ...
        if (matchingSession) {
          try {
            ...
            console.error(
              `[sandbox-import] Failed to write Claude session:`,
              sessionErr,
            );
          }
        }
  ...
  listRemoteSandboxChats: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch sandbox chats: ${response.statusText}`,
        );
      }
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        if (debugResponse.ok) {
          ...
          console.log(
            `[OPEN-LOCALLY] ========== SANDBOX DEBUG INFO ==========`,
          );
          console.log(`[OPEN-LOCALLY] Paths:`, debugData.paths);
          console.log(`[OPEN-LOCALLY] Checks:`, debugData.checks);
          console.log(
            `[OPEN-LOCALLY] Files in .claude:`,
            debugData.files?.claudeHome,
          );
          console.log(
            `[OPEN-LOCALLY] Projects dirs:`,
            debugData.files?.projects,
          );
          console.log(
            `[OPEN-LOCALLY] Project dir contents:`,
            debugData.files?.projectDir,
          );
          console.log(
            `[OPEN-LOCALLY] Sessions index:`,
            debugData.sessionsIndex,
          );
          console.log(
            `[OPEN-LOCALLY] Session files exist:`,
            debugData.sessionFilesExist,
          );
          console.log(`[OPEN-LOCALLY] Errors:`, debugData.errors);
          console.log(`[OPEN-LOCALLY] ========== END SANDBOX DEBUG ==========`);
        } else {
          console.log(
            `[OPEN-LOCALLY] Debug endpoint returned ${debugResponse.status}`,
          );
        }
      } catch (debugErr) {
      ...
      console.log(
        `[OPEN-LOCALLY] Creating target directory: ${input.targetPath}`,
      );
      ...
      if (!finalOwner || !finalRepo) {
        ...
        if (repoFromMeta) {
          ...
          if (metaOwner && metaRepo) {
            console.log(
              `[OPEN-LOCALLY] Git remote missing, using meta.repository: ${repoFromMeta}`,
            );
            ...
            try {
              await execAsync(`git remote add origin ${finalRemoteUrl}`, {
                cwd: input.targetPath,
              });
              console.log(
                `[OPEN-LOCALLY] Added origin remote: ${finalRemoteUrl}`,
              );
            } catch (err) {
              ...
              try {
                ...
                console.log(
                  `[OPEN-LOCALLY] Updated origin remote: ${finalRemoteUrl}`,
                );
              } catch {
      ...
      console.log(
        `[OPEN-LOCALLY] Final git info: owner="${finalOwner}", repo="${finalRepo}"`,
      );
      ...
      console.log(
        `[OPEN-LOCALLY] Checking for existing project at path: ${input.targetPath}`,
      );
      ...
      console.log(`[OPEN-LOCALLY] Project created/updated:`, {
        id: project.id,
        name: project.name,
      });
      ...
      console.log(
        `[OPEN-LOCALLY] Creating chat record with branch: ${actualBranch}`,
      );
      const chat = db
        .insert(chats)
        .values({
      ...
      console.log(`[OPEN-LOCALLY] Chat created:`, {
        id: chat.id,
        name: chat.name,
      });
      ...
      // Import sub-chats with messages and Claude sessions
      console.log(
        `[OPEN-LOCALLY] Importing ${remoteChatData.subChats.length} sub-chats...`,
      );
      const claudeSessions = importResult.claudeSessions || [];
      console.log(
        `[OPEN-LOCALLY] Available Claude sessions: ${claudeSessions.length}`,
      );

      for (const remoteSubChat of remoteChatData.subChats) {
        const messagesArray = remoteSubChat.messages || [];
        const messagesCount = Array.isArray(messagesArray)
        ...
        console.log(
          `[OPEN-LOCALLY] Messages preview:`,
          JSON.stringify(messagesArray).slice(0, 500),
        );
        ...
        if (matchingSession) {
          try {
            ...
            console.error(
              `[OPEN-LOCALLY] Failed to write Claude session:`,
              sessionErr,
            );
          }
          ...
          console.log(
            `[OPEN-LOCALLY] No sessionId in messages or no sessions exported`,
          );
        }
      }
