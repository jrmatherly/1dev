  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    ...
    if (
      ...
      (eventTimestampMs === undefined ||
        eventTimestampMs < notBeforeTimestampMs)
    ) {
      continue;
    }
    ...
    if (rawTokenUsage && typeof rawTokenUsage === "object") {
      ...
      if (
        Object.values(parsedTokenUsage).some(
    ...
    if (!info.last_token_usage && info.model_context_window === undefined)
      continue;
...
function mapToUsageMetadata(
  ...
  const inputTokens =
    perMessageUsage?.input_tokens !== undefined
      ? Math.max(
          0,
          perMessageUsage.input_tokens -
            (perMessageUsage.cached_input_tokens ?? 0),
        )
      : undefined;
  ...
  const totalTokens =
    ...
    (perMessageUsage?.input_tokens !== undefined ||
    perMessageUsage?.output_tokens !== undefined
      ? (perMessageUsage?.input_tokens ?? 0) +
        (perMessageUsage?.output_tokens ?? 0)
      : undefined);
...
function buildCodexProviderEnv(authConfig?: {
  apiKey: string;
