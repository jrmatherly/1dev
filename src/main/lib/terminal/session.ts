...
function validateAndResolveCwd(cwd: string): string {
  if (!fs.existsSync(cwd)) {
    ...
    console.warn(
      `[Terminal] CWD does not exist: ${cwd}, using home directory: ${homeDir}`,
    );
    return homeDir;
  }
  ...
  try {
    ...
    if (!stat.isDirectory()) {
      const homeDir = os.homedir();
      console.warn(
        `[Terminal] CWD is not a directory: ${cwd}, using home directory: ${homeDir}`,
      );
      return homeDir;
    }
  } catch {
    const homeDir = os.homedir();
    console.warn(
      `[Terminal] Error checking CWD: ${cwd}, using home directory: ${homeDir}`,
    );
    return homeDir;
  }
  ...
  try {
    ...
    const homeDir = os.homedir();
    console.warn(
      `[Terminal] Error resolving CWD: ${cwd}, using home directory: ${homeDir}`,
    );
    return homeDir;
  }
}
...
async function spawnPty(params: {
  ...
  try {
    ...
    console.error(
      `[Terminal] Failed to spawn PTY with ${resolvedShell}:`,
      error,
    );
