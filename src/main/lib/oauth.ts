...
function generateOAuthPage(options: {
  ...
  const terminalLines: TerminalLine[] = isSuccess
    ...
    : [
        ...
        ...(errorDetail
          ? [{ text: `error: ${errorDetail}`, isError: true }]
          : []),
      ];
  ...
  const terminalLinesHtml = terminalLines
    .map((line, i) => {
      ...
      if (line.isHighlight) {
        const color =
          line.highlightColor === "green" ? "var(--green)" : "var(--red)";
        const glow =
          line.highlightColor === "green"
...
export class CraftOAuth {
  ...
  async checkAuthRequired(): Promise<boolean> {
    ...
    try {
      ...
      this.callbacks.onStatus(
        "Could not reach OAuth metadata - assuming public",
      );
      return false;
    }
  }
  ...
  async authenticate(): Promise<{ tokens: OAuthTokens; clientId: string }> {
    ...
    try {
      ...
      this.callbacks.onStatus(
        `Found OAuth endpoints at ${this.config.mcpBaseUrl}`,
      );
    } catch (error) {
