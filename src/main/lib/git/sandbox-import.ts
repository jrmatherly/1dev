...
export async function parseExportStream(
  ...
  while (true) {
    ...
    if (done) {
      console.log(
        `[sandbox-import] Stream finished, processed ${chunkCount} chunks`,
      );
      break;
    }
    ...
    for (const line of lines) {
      ...
      chunkCount++;
      console.log(
        `[sandbox-import] Received chunk ${chunkCount}: type=${chunk.type}`,
      );
      ...
      switch (chunk.type) {
        ...
        case "bundle":
          if (chunk.data) {
            ...
            console.log(
              `[sandbox-import] Bundle chunk: ${result.bundle.length} bytes`,
            );
          } else {
        ...
        case "staged_patch":
          ...
          console.log(
            `[sandbox-import] Staged patch chunk: ${chunk.data?.length || 0} chars`,
          );
          break;
        case "unstaged_patch":
          result.unstagedPatch = chunk.data;
          console.log(
            `[sandbox-import] Unstaged patch chunk: ${chunk.data?.length || 0} chars`,
          );
          break;
        case "untracked":
        ...
        case "error":
          console.error(
            `[sandbox-import] Error chunk received: ${chunk.error}`,
          );
...
export async function applySandboxGitState(
  ...
  try {
    ...
    if (isFullExport && exportData.meta.remoteUrl) {
      console.log(
        `[sandbox-import] Setting up remote origin: ${exportData.meta.remoteUrl}`,
      );
      try {
        ...
        console.log(
          `[sandbox-import] Remote origin already exists or failed to add:`,
          err,
        );
      }
    }
    ...
    if (!isFullExport) {
      ...
      try {
        ...
        console.log(
          `[sandbox-import] Base commit ${baseCommit} not found locally, fetching...`,
        );
        try {
    ...
    if (exportData.bundle) {
      ...
      try {
        ...
        console.log(
          `[sandbox-import] Bundle verify output:`,
          verifyResult.stdout,
        );
        ...
        if (isFullExport) {
          ...
          console.log(
            `[sandbox-import] Full export: fetching all refs from bundle...`,
          );
          ...
          console.log(
            `[sandbox-import] Checked out branch successfully: ${targetBranch}`,
          );
        } else {
    ...
    if (exportData.unstagedPatch) {
      ...
      const unstagedPatchPath = join(
        tmpdir(),
        `sandbox-unstaged-${Date.now()}.patch`,
      );
      try {
        ...
        await execFileAsync(
          "git",
          ["-C", worktreePath, "apply", unstagedPatchPath],
          {
            timeout: 60_000,
          },
        );
        ...
        console.warn(
          `[sandbox-import] Failed to apply unstaged patch: ${error}`,
        );
      } finally {
    ...
    for (const file of exportData.untrackedFiles) {
      ...
      try {
        ...
        console.warn(
          `[sandbox-import] Failed to write untracked file ${file.path}: ${error}`,
        );
      }
    }

    console.log(
      `[sandbox-import] applySandboxGitState completed successfully!`,
    );
    return { success: true };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error(
      `[sandbox-import] applySandboxGitState FAILED: ${errorMessage}`,
    );
...
export async function importSandboxToWorktree(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Export API returned ${response.status}: ${response.statusText}`,
      );
    }
    ...
    // Apply the git state
    console.log(
      `[sandbox-import] Applying git state to worktree: ${worktreePath}`,
    );
