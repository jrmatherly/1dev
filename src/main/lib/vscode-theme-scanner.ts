...
const EXTENSION_PATHS: { path: string; source: EditorSource }[] = [
  ...
  // VS Code Insiders
  {
    path: path.join(os.homedir(), ".vscode-insiders", "extensions"),
  ...
  // Windsurf
  {
    path: path.join(os.homedir(), ".windsurf", "extensions"),
    source: "windsurf",
  },
];
...
async function scanExtensionsDir(
  ...
  try {
    ...
    const lsOutput = execSync(`ls -1 "${extensionsDir}"`, {
      encoding: "utf-8",
    });
    ...
    console.error(
      `Error scanning extensions directory ${extensionsDir}:`,
      error,
    );
  }
