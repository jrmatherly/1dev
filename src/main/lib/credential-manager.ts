...
export class SourceCredentialManager {
  ...
  async save(
    ...
    debug(
      `[SourceCredentialManager] Saved ${credentialId.type} for ${source.config.slug}`,
    );
  }
  ...
  async load(source: LoadedSource): Promise<StoredCredential | null> {
    ...
    if (cred) {
      debug(
        `[SourceCredentialManager] Found ${credentialId.type} for ${source.config.slug}`,
      );
    }
  ...
  private async loadMcpCredential(
    ...
    if (oauthCreds?.value) {
      debug(
        `[SourceCredentialManager] Found source_oauth for ${source.config.slug}`,
      );
      return oauthCreds;
    }
    ...
    if (bearerCreds?.value) {
      debug(
        `[SourceCredentialManager] Found source_bearer for ${source.config.slug}`,
      );
      return bearerCreds;
    }

    debug(
      `[SourceCredentialManager] No credential found for MCP source ${source.config.slug}`,
    );
    return null;
  }
  ...
  async delete(source: LoadedSource): Promise<boolean> {
    ...
    if (deleted) {
      debug(
        `[SourceCredentialManager] Deleted ${credentialId.type} for ${source.config.slug}`,
      );
    }
    return deleted;
  }
  ...
  async getToken(source: LoadedSource): Promise<string | null> {
    ...
    if (this.isExpired(cred)) {
      debug(
        `[SourceCredentialManager] Token expired for ${source.config.slug}`,
      );
      return null;
    }
  ...
  async refresh(source: LoadedSource): Promise<string | null> {
    ...
    if (!cred?.refreshToken) {
      debug(
        `[SourceCredentialManager] No refresh token for ${source.config.slug}`,
      );
      return null;
    }
  ...
  private async refreshGoogle(
    ...
    try {
      ...
      debug(
        `[SourceCredentialManager] Refreshed Google token for ${source.config.slug}`,
      );
  ...
  private async refreshSlack(
    ...
    try {
      ...
      debug(
        `[SourceCredentialManager] Refreshed Slack token for ${source.config.slug}`,
      );
      return result.accessToken;
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
  ...
  private async refreshMicrosoft(
    ...
    try {
      ...
      debug(
        `[SourceCredentialManager] Refreshed Microsoft token for ${source.config.slug}`,
      );
      return result.accessToken;
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : String(error);
  ...
  private async refreshMcp(
    ...
    try {
      ...
      if (!source.config.mcp?.url) {
        ...
        debug(
          `[SourceCredentialManager] No URL for MCP token refresh (stdio transport)`,
        );
        return null;
      }
      ...
      debug(
        `[SourceCredentialManager] Refreshed MCP token for ${source.config.slug}`,
      );
