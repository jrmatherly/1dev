...
function normalizeCodexToolInput(
  ...
  if (!isRecord(rawInput)) {
    ...
    if (
      (descriptor.canonicalToolName === "Grep" ||
        descriptor.canonicalToolName === "Glob") &&
      descriptor.detail
    ) {
...
export function normalizeCodexToolPart(
  ...
  const normalizedType = descriptor
    ? `tool-${descriptor.canonicalToolName}`
    : part.type;
  ...
  const outputEnrichedInput =
    fallbackDescriptor.canonicalToolName === "Read"
      ...
      : normalizedInput;
  const finalInput =
    outputEnrichedInput !== part.input &&
    isShallowEqual(outputEnrichedInput, part.input)
      ? part.input
      : outputEnrichedInput;
...
export function normalizeCodexStreamChunk(chunk: unknown): unknown {
  ...
  const normalizedTitle =
    typeof chunk.title === "string" && chunk.title.trim().length > 0
      ? chunk.title
      : typeof descriptor?.detail === "string" &&
          descriptor.detail.trim().length > 0
        ? descriptor.detail
        : undefined;
  const finalInput =
    chunk.type === "tool-input-available" &&
  ...
  const titleChanged =
    normalizedTitle !== undefined && normalizedTitle !== chunk.title;
  const inputChanged =
