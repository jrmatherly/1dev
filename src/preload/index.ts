...
contextBridge.exposeInMainWorld("desktopApi", {
  ...
  getUpdateChannel: () =>
    ipcRenderer.invoke("update:get-channel") as Promise<"latest" | "beta">,
  ...
  // Analytics
  setAnalyticsOptOut: (optedOut: boolean) =>
    ipcRenderer.invoke("analytics:set-opt-out", optedOut),
  ...
  onStreamChunk: (streamId: string, callback: (chunk: Uint8Array) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(`stream:${streamId}:chunk`, handler);
  },
  ...
  onStreamError: (streamId: string, callback: (error: string) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(`stream:${streamId}:error`, handler);
  },
