...
function getServersFromContext(context: ToolsSearchContext): ToolData[] {
  ...
  const connectedServers = context.mcpServers.filter(
  ...
  if (context.mcpTools) {
    for (const tool of context.mcpTools) {
      ...
      toolCountByServer.set(
        serverName,
        (toolCountByServer.get(serverName) || 0) + 1,
      );
    }
  }
...
export const toolsProvider = createMentionProvider<ToolData>({
  ...
  ): Promise<MentionSearchResult<ToolData>> {
  ...
  deserialize(token: string): MentionItem<ToolData> | null {
    try {
      ...
      console.warn(
        `[ToolsProvider] Failed to deserialize token: ${token}`,
        error,
      );
      return null;
    }
