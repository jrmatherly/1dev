...
export const filesProvider = createMentionProvider<FileData>({
  ...
  async search(
  ...
  ): Promise<MentionSearchResult<FileData>> {
    ...
    try {
      ...
      if (
        ...
        const changedItems: MentionItem<FileData>[] = context.changedFiles.map(
          (file) => ({
  ...
  deserialize(token: string): MentionItem<FileData> | null {
    try {
      ...
      if (
        !token.startsWith(MENTION_PREFIXES.FILE) &&
        !token.startsWith(MENTION_PREFIXES.FOLDER)
      ) {
        return null;
      }
      ...
      console.warn(
        `[FilesProvider] Failed to deserialize token: ${token}`,
        error,
      );
      return null;
    }
