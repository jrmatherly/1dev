...
export function useCommitActions({
  ...
  const commit = useCallback(
    async ({ message, filePaths }: CommitActionInput): Promise<boolean> => {
      ...
      console.log(
        "[CommitActions] commit called, commitMessage:",
        commitMessage,
        "chatId:",
        chatId,
      );
      ...
      if (!commitMessage && chatId) {
        console.log(
          "[CommitActions] No message, generating with AI for files:",
          filePaths,
        );
  ...
  const isPending =
    isGenerating || atomicCommitMutation.isPending || commitMutation.isPending;
