...
import {
  ChangesFileFilter,
  type SubChatFilterItem,
} from "./components/changes-file-filter";
...
export function ChangesView({
  ...
  const discardMultipleChangesMutation =
    trpc.changes.discardMultipleChanges.useMutation({
      onSuccess: () => {
        toast.success("Changes discarded");
        refetch();
        onDiscardSuccess?.();
  ...
  const deleteMultipleUntrackedMutation =
    trpc.changes.deleteMultipleUntracked.useMutation({
      onSuccess: () => {
  ...
  const [subChatFilter, setSubChatFilter] = useState<string | null>(
  ...
  const [selectedForCommit, setSelectedForCommit] = useState<Set<string>>(
    new Set(),
  );
  ...
  const selectedCount = filteredFiles.filter((f) =>
    selectedForCommit.has(f.file.path),
  ).length;
  ...
  return (
    <>
          ...
          >
            ...
            <div className="flex items-center gap-2 px-2 py-1.5 border-b border-border/50">
              ...
              <span className="text-xs text-muted-foreground">
                {selectedCount} of {totalCount} file
              ...
              >
                {filteredFiles.map(({ file, category }, index) => (
                  <ChangesFileItemWithContext
                    ...
                    onOpenInFilePreview={() =>
                      handleOpenInFilePreview(file.path)
                    }
                    onOpenInEditor={() =>
                      handleOpenInPreferredEditor(file.path)
                    }
                    ...
                    onCopySelectedPaths={() =>
                      handleCopySelectedPaths(worktreePath)
                    }
                    onCopySelectedRelativePaths={
      ...
      <AlertDialog
        open={!!discardFile}
        onOpenChange={(open) => !open && setDiscardFile(null)}
      >
        <AlertDialogContent className="w-[340px]">
          <AlertDialogHeader>
            <AlertDialogTitle>
              {discardFile?.status === "untracked" ||
              discardFile?.status === "added"
      ...
      <AlertDialog
        open={!!discardFiles}
        onOpenChange={(open) => !open && setDiscardFiles(null)}
      >
