...
export const ForkContext = createContext<((messageId: string) => void) | null>(
  null,
);
...
export const IsolatedMessageGroup = memo(function IsolatedMessageGroup({
  ...
  const userMsg = useAtomValue(
    messageAtomFamily(getPerChatMessageKey(subChatId, userMsgId)),
  );
  const assistantIds = useAtomValue(assistantIdsPerChatAtomFamily(perChatKey));
  const isLastGroup = useAtomValue(
    isLastUserMessagePerChatAtomFamily(perChatKey),
  );
  const rollbackTargetSdkUuid = useAtomValue(
    rollbackTargetPerChatAtomFamily(perChatKey),
  );
  const subChatStatus = useStreamingStatusStore(
    useCallback((state) => state.statuses[subChatId] ?? "ready", [subChatId]),
  );
  const isStreaming =
    subChatStatus === "streaming" || subChatStatus === "submitted";
  ...
  return (
    <MessageGroupWrapper isLastGroup={isLastGroup}>
      ...
      {((!isImageOnlyMessage && imageParts.length > 0) ||
