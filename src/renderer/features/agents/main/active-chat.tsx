...
const pendingSubChatCleanupTimers = new Map<
  string,
  ...
  const handleForkFromMessage = useCallback(
    async (messageId: string) => {
      ...
      try {
        ...
        const messageIndex = messagesForForkRef.current.findIndex(
          (m) => m.id === messageId,
        );
        ...
        {!isMobile && (
          ...
          >
            <ChatTitleEditor
              ...
              hasMessages={
                true
              } /* Always show "New Chat" placeholder when name is empty */
            />
...
export function ChatView({
  ...
  useEffect(() => {
    ...
    const isFullPageFileViewer =
      !!fileViewerPath && fileViewerDisplayMode === "full-page";
    const shouldHide = isFullPageDiff || isFullPageFileViewer;
    window.desktopApi.setTrafficLightVisibility(
      shouldHide ? false : sidebarOpen,
    );
  }, [
    isDiffSidebarOpen,
    diffDisplayMode,
  ...
  const tabsToRender = useMemo(() => {
    ...
    const validSplitPaneIds = splitPaneIds.filter((id) =>
      validSubChatIds.has(id),
    );
  ...
  useEffect(() => {
    ...
    if (previousParentChatId && previousParentChatId !== chatId) {
      for (const subChatId of agentChatStore.keys()) {
        if (agentChatStore.getParentChatId(subChatId) !== previousParentChatId)
          continue;
  ...
  useEffect(() => {
    ...
    for (const subChatId of agentChatStore.keys()) {
      ...
      if (useStreamingStatusStore.getState().isStreaming(subChatId)) continue;
      if ((useMessageQueueStore.getState().queues[subChatId]?.length ?? 0) > 0)
        continue;
...
Make sure to preserve all functionality from both branches when resolving conflicts.`;
  ...
  const {
    ...
    { worktreePath: worktreePath || "" },
    {
      enabled: !!worktreePath && (isDiffSidebarOpen || isDetailsSidebarOpen),
      staleTime: 30000,
    },
  );
  ...
  const handleCreateNewSubChat = useCallback(async () => {
    ...
    const newSubChatProvider = inferProviderFromMessages(
                              ...
                              return (
                                ...
                                >
                                  <ChatViewInner
                                    ...
                                    subChatId={subChatId}
                                    parentChatId={chatId}
                                    provider={inferProviderFromMessages(
                                      subChatId,
                                    )}
                                    isFirstSubChat={isFirstSubChat}
                                    onAutoRename={handleAutoRename}
                                    ...
                                    teamId={selectedTeamId || undefined}
                                    repository={repository}
                                    streamId={agentChatStore.getStreamId(
                                      subChatId,
                                    )}
                                    isMobile={isMobileFullscreen}
                                    isSubChatsSidebarOpen={
            ...
            {isUnifiedSidebarEnabled &&
              ...
              (worktreePath || sandboxId) && (
                <DetailsSidebar
                  ...
                  onCommitAndPush={
                    worktreePath ? handleCommitAndPush : undefined
                  }
