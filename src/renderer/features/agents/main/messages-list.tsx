...
export function useStreamingStatus() {
  ...
  if (!store)
    throw new Error(
      "useStreamingStatus must be used within MessageStoreProvider",
    );
...
function useIsLastMessage(messageId: string) {
  ...
  if (!store)
    throw new Error(
      "useIsLastMessage must be used within MessageStoreProvider",
    );
...
function useIsStreaming() {
  ...
  const cacheRef = useRef<{ isStreaming: boolean; status: string } | null>(
    null,
  );
...
function useMessageWithLastStatus(messageId: string) {
  ...
  if (!store)
    throw new Error(
      "useMessageWithLastStatus must be used within MessageStoreProvider",
    );

  // Track what we last returned to detect changes
  const lastReturnedRef = useRef<{ message: any; isLast: boolean } | null>(
    null,
  );
...
export const MemoizedAssistantMessages = memo(
  function MemoizedAssistantMessages({
    assistantMsgIds,
    subChatId,
    chatId,
    isMobile,
...
export function useMessageGroups() {
  ...
  if (!store)
    throw new Error(
      "useMessageGroups must be used within MessageStoreProvider",
    );
    ...
    if (groups.length !== groupsCacheRef.current.length) {
      ...
      for (let i = 0; i < groups.length; i++) {
        ...
        if (
          ...
          assistantIdsCacheRef.current.set(
            newGroup.userMsg.id,
            newGroup.assistantMsgIds,
          );
        }
...
export function useUserMessageIds() {
  ...
  if (!store)
    throw new Error(
      "useUserMessageIds must be used within MessageStoreProvider",
    );
...
export function useUserMessageWithAssistants(userMsgId: string) {
  ...
  if (!store)
    throw new Error(
      "useUserMessageWithAssistants must be used within MessageStoreProvider",
    );
  ...
  const subscribe = useCallback(
    (onStoreChange: () => void) => {
      return store.subscribe(() => {
        ...
        if (cacheRef.current) {
          const idsChanged =
            assistantMsgIds.length !==
              cacheRef.current.assistantMsgIds.length ||
            !assistantMsgIds.every(
              (id, i) => id === cacheRef.current!.assistantMsgIds[i],
            );
...
export const SimpleIsolatedGroup = memo(function SimpleIsolatedGroup({
  ...
  const { userMsg, assistantMsgIds, isLastGroup } =
    useUserMessageWithAssistants(userMsgId);
            ...
            <div className="space-y-2 w-full">
              <div className="bg-input-background border px-3 py-2 rounded-xl text-sm text-muted-foreground italic">
                {(() => {
                  ...
                  const quoteCount = textMentions.filter(
                    (m) => m.type === "quote",
                  ).length;
                  const pastedCount = textMentions.filter(
                    (m) => m.type === "pasted",
                  ).length;
                  const codeCount = textMentions.filter(
                    (m) => m.type === "diff",
                  ).length;
                  if (quoteCount > 0) {
                    parts.push(
...
export const SimpleIsolatedMessagesList = memo(
  function SimpleIsolatedMessagesList({
    subChatId,
    isMobile,
    sandboxSetupStatus,
