...
function MentionChip({ mention }: { mention: ParsedMention }) {
  ...
  if (mention.type === "quote") {
    ...
    const displayTitle =
      title.length < mention.label.length ? `${title}...` : title;
  ...
  const isClickable =
    (mention.type === "file" || mention.type === "folder") && !!onOpenFile;
...
export function hasFileMentions(text: string): boolean {
  return /@\[(file|folder|skill|agent|tool|quote|diff|pasted|chatHistory):[^\]]+\]/.test(
    text,
  );
}
