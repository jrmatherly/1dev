...
> {
  ...
  static getDerivedStateFromError(
    error: Error,
  ): Partial<DiffErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      if (this.props.rawDiff) {
        ...
        const contentLines =
          firstHunkIdx > 0 ? lines.slice(firstHunkIdx) : lines;
          ...
          <div className="text-xs font-mono overflow-x-auto">
            {contentLines.map((line, i) => {
              ...
              if (line.startsWith("+") && !line.startsWith("+++")) {
                ...
                className +=
                  " text-muted-foreground bg-blue-500/5 py-1 mt-1 first:mt-0";
              }
              return (
...
const FileDiffCard = memo(function FileDiffCard({
  ...
  const isLargeDiff =
    file.additions + file.deletions >= LARGE_DIFF_LINE_THRESHOLD;
  ...
  const [fileDiffMeta, setFileDiffMeta] = useState<ReturnType<
    typeof parseDiffFromFile
  > | null>(null);
      ...
      {worktreePath ? (
        <ContextMenu>
          ...
          <ContextMenuContent className="w-56">
            ...
            <ContextMenuSeparator />
            <ContextMenuItem
              onClick={handleOpenInFilePreview}
              className="text-xs"
            >
...
export const AgentDiffView = forwardRef<AgentDiffViewRef, AgentDiffViewProps>(
  function AgentDiffView(
    ...
    const [viewedFiles, setViewedFiles] = useAtom(
      viewedFilesAtomFamily(chatId),
    );
    ...
    const fileDiffs = useMemo(() => {
      ...
      const validFilterPaths =
        effectiveFilter?.filter((p) => p && p !== "/dev/null") || [];
    ...
    const getViewedCount = useCallback(() => {
      return fileDiffs.filter((file) => isFileViewed(file.key, file.diffText))
        .length;
    ...
    const prevCollapseStateRef = useRef<{
      allCollapsed: boolean;
      allExpanded: boolean;
    } | null>(null);
    useEffect(() => {
    ...
    useEffect(() => {
      ...
      for (const file of fileDiffs) {
        ...
        if (
          viewedState?.viewed &&
          viewedState.contentHash !== hashString(file.diffText)
        ) {
    ...
    useEffect(() => {
      ...
      if (
        currentFileKeys !== prevFileKeysRef.current &&
            ...
            const results = await Promise.allSettled(
              filesToFetch.map(async ({ key, filePath }) => {
                if (isDesktopApp()) {
                  ...
                  const data = await remoteApi.getSandboxFile(
                    sandboxId,
                    filePath,
                  );
    ...
    const handleToggleViewed = useCallback(
      (fileKey: string, diffText: string) => {
        ...
        const isFileViewedWithNewState = (
          fKey: string,
          fDiffText: string,
        ): boolean => {
        ...
        if (willBeViewed && onSelectNextFile) {
          ...
          if (nextUnviewedFile) {
            ...
            const filePath =
              ...
              nextUnviewedFile.newPath !== "/dev/null"
                ? nextUnviewedFile.newPath
    ...
    const totalAdditions = allFileDiffs.reduce(
      (sum, f) => sum + f.additions,
      0,
    );
    const totalDeletions = allFileDiffs.reduce(
        ...
        >
          <AlertDialogContent className="w-[340px]">
            ...
            <AlertDialogDescription className="px-5 pb-5">
              This will revert all changes to this file. This action cannot be
              undone.
            </AlertDialogDescription>
            <AlertDialogFooter>
              <Button
