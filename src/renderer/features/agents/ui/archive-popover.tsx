        ...
        <div className="flex-1 min-w-0 flex flex-col gap-0.5">
          <div className="flex items-center gap-1">
            <span className="truncate block text-sm leading-tight flex-1">
              {chat.name || (
                <span className="text-muted-foreground/50">New workspace</span>
              )}
            </span>
            <button
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1 text-[11px] text-muted-foreground/60 truncate min-w-0">
              ...
              {chat.isRemote && <CloudIcon className="h-2.5 w-2.5 shrink-0" />}
...
export const ArchivePopover = memo(function ArchivePopover({
  ...
  const { data: remoteArchivedChats, isLoading: isRemoteLoading } =
    useRemoteArchivedChats();
    ...
    if (remoteArchivedChats) {
      for (const chat of remoteArchivedChats) {
        ...
        merged.push({
          ...
          archivedAt:
            (chat as unknown as { archived_at?: string }).archived_at ?? null,
          isRemote: true,
        });
      }
    }
