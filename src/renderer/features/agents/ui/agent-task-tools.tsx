...
const snapshotHistoryCache = new Map<
  string,
  Map<string, Map<string, TaskSnapshot>>
>();
...
function extractChangesFromParts(
  ...
  if (!parts || !Array.isArray(parts))
    return { changes: [], taskSubjects: new Map() };
  ...
  for (const part of parts) {
    ...
    if (part.type === "tool-TaskUpdate") {
      ...
      const subject = inputSubject
        ...
        : (taskSubjects.get(taskId) ??
          formatTaskSubject(taskId, `Task #${taskId}`));
      ...
      if (existingIdx === -1) {
        ...
        const isStatusChange =
          statusChange || (inputStatus && inputStatus !== "deleted");
...
function getChangeDescription(change: TaskChange): string | null {
  switch (change.changeType) {
    ...
    case "updated":
      ...
      if (
        change.updatedFields?.includes("blockedBy") ||
        change.updatedFields?.includes("blocks")
      ) {
...
const ChangeItem = memo(function ChangeItem({
  ...
  const displayText =
    change.newStatus === "in_progress" && change.activeForm
      ? change.activeForm
      : change.subject;
...
export const AgentTaskToolsGroup = memo(function AgentTaskToolsGroup({
  ...
  const { previousSnapshot, currentSnapshot } = useMemo(() => {
    ...
    const currentSnapshot = updateTaskSnapshotFromParts(
      parts,
      previousSnapshot,
    );
  ...
  const { taskList, taskGet } = useMemo(
    () => extractReadDataFromParts(parts),
    [parts],
  );
