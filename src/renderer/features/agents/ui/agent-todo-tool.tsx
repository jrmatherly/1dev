...
export const AgentTodoTool = memo(function AgentTodoTool({
  ...
  useEffect(() => {
    if (newTodos.length > 0) {
      ...
      const shouldUpdate =
        isCreationToolCallLocal || newTodos.length >= currentSyncedTodos.length;
      ...
      if (shouldUpdate) {
        ...
        if (
          newTodosJson !== syncedTodosJson ||
          currentCreationToolCallId !== newCreationId
        ) {
  ...
  if (
    changes.type === "single" &&
    !(alwaysExpandTodoList && isCreationToolCall)
  ) {
    const change = changes.items[0];
    // Use stable icon reference from TOOL_CALL_ICONS map
    const IconComponent =
      TOOL_CALL_ICONS[change.newStatus] || TOOL_CALL_ICONS.pending;
