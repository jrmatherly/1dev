              ...
              <div className="bg-background rounded-2xl border shadow-2xl overflow-hidden">
                <div className="p-6">
                  ...
                  <p className="text-sm text-muted-foreground">
                    To use a different agent, a new chat will be created with
                    your current conversation history attached.
                  </p>
                </div>
...
export function AgentModelSelector({
  ...
  const isItemSelected = (item: FlatModelItem): boolean => {
    switch (item.type) {
      ...
      case "codex":
        return (
          selectedAgentId === "codex" && codex.selectedModelId === item.model.id
        );
      case "ollama":
        return (
          selectedAgentId === "claude-code" &&
  ...
  const isItemCrossProvider = (item: FlatModelItem): boolean => {
    return (
      !canSelectProvider(getItemProvider(item)) && !!onContinueWithProvider
    );
  };
