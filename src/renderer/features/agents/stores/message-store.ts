...
const messageRolesAtom = atom<Map<string, "user" | "assistant" | "system">>(
  new Map(),
);
...
export const messageStructureAtomFamily = atomFamily((messageKey: string) =>
  atom((get) => {
    ...
    const partsStructure: PartStructure[] = (message.parts || []).map(
      (part: any) => {
  ...
  if (groups.length === cachedMessageGroups.length) {
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      if (
        ...
        newGroup.assistantMsgIds.length !==
          cachedGroup?.assistantMsgIds.length ||
        !newGroup.assistantMsgIds.every(
          (id, j) => id === cachedGroup?.assistantMsgIds[j],
        )
      ) {
        allMatch = false;
        break;
...
export const assistantIdsForUserMsgAtomFamily = atomFamily(
  (userMsgId: string) =>
    atom((get) => {
...
export const rollbackTargetSdkUuidForUserMsgAtomFamily = atomFamily(
  (userMsgId: string) =>
    atom((get) => {
      const ids = get(messageIdsAtom);
      const roles = get(messageRolesAtom);
...
const lastAssistantCacheByChat = new Map<
  string,
  { id: string | null; msg: Message | null }
>();
...
export const lastAssistantMessageAtom = atom((get) => {
  ...
  if (lastAssistantId === cached?.id && cached.msg) {
    ...
    const freshMsg = get(
      messageAtomFamily(getPerChatMessageKey(subChatId, lastAssistantId)),
    );
    if (freshMsg === cached.msg) {
      return cached.msg;
    }
    lastAssistantCacheByChat.set(subChatId, {
      id: lastAssistantId,
      msg: freshMsg,
    });
    return freshMsg;
...
export const syncMessagesWithStatusAtom = atom(
  ...
  (
    ...
    const previousIds =
      activeMessageIdsByChat.get(currentSubChatId) ?? new Set();
    for (const oldId of previousIds) {
      if (!newIdsSet.has(oldId)) {
        ...
        isLastUserMessagePerChatAtomFamily.remove(
          `${currentSubChatId}:${oldId}`,
        );
