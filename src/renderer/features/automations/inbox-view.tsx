  ...
  return (
    <ContextMenu>
      ...
      <ContextMenuContent className="w-48">
        ...
        <ContextMenuSeparator />
        <ContextMenuItem
          onClick={(e) => onArchive(e as unknown as React.MouseEvent)}
        >
          Archive
        </ContextMenuItem>
...
export function InboxView() {
  ...
  const [filterMode, setFilterMode] = useState<
    "unread_and_read" | "unread" | "archived" | "all"
  >("unread_and_read");
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      remoteTrpc.automations.getInboxChats.query({
        teamId: teamId!,
        limit: 50,
      }),
    enabled: !!teamId,
  ...
  const markReadMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["automations", "inboxUnreadCount"],
      });
      queryClient.invalidateQueries({
        queryKey: ["automations", "inboxChats"],
      });
    },
  });
  ...
  const [importingRemoteChat, setImportingRemoteChat] =
    useState<RemoteChat | null>(null);
  ...
  const handleForkLocally = useCallback(
    async (chatId: string) => {
      try {
        ...
        console.error(
          "[InboxView] Failed to fetch chat for fork locally:",
          err,
        );
      }
    },
  ...
  const handleArchiveBelow = useCallback(
    (chatId: string) => {
      ...
      if (
        selectedChatId &&
        filteredChats.findIndex((c) => c.id === selectedChatId) > index
      ) {
        setSelectedChatId(chatId);
      }
    },
  ...
  const filterOptions = [
    {
      value: "unread_and_read" as const,
      label: "Unread & read",
      icon: InboxIcon,
    },
