...
export function AutomationsDetailView() {
  ...
  const { data: automation, isLoading } = useQuery({
    ...
    queryFn: () =>
      remoteTrpc.automations.getAutomation.query({
  ...
  const { data: moreExecutionsData, isFetching: isFetchingMoreExecutions } =
    useQuery({
  ...
  useEffect(() => {
    if (moreExecutionsData?.executions) {
      setAdditionalExecutions((prev) => [
        ...prev,
        ...moreExecutionsData.executions,
      ]);
    }
  }, [moreExecutionsData]);
  ...
  const deleteMutation = useMutation({
    mutationFn: () =>
      remoteTrpc.automations.deleteAutomation.mutate({
        automationId: automationId!,
      }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["automations", "list"] });
  ...
  const handleAddTrigger = useCallback((platform: Platform) => {
    const defaultTrigger =
      platform === "github" ? "pr_opened" : "linear_issue_created";
  ...
  const handleUpdateTriggerType = useCallback(
    (triggerId: string, triggerType: TriggerType) => {
      setLocalTriggers((prev) =>
        prev.map((t) =>
          t.id === triggerId ? { ...t, trigger_type: triggerType } : t,
        ),
      );
    },
    [],
                ...
                {localTriggers.map((trigger) => (
                  ...
                  >
                    ...
                    <Select
                      ...
                      onValueChange={(v) =>
                        handleUpdateTriggerType(trigger.id, v as TriggerType)
                      }
                    >
                ...
                <DropdownMenu>
                  ...
                  >
                    {githubStatus?.isConnected && (
                      <DropdownMenuItem
                        onClick={() => handleAddTrigger("github")}
                      >
                        <PlatformIcon
                          platform="github"
                    ...
                    {linearStatus?.isConnected && (
                      <DropdownMenuItem
                        onClick={() => handleAddTrigger("linear")}
                      >
                        <PlatformIcon
                          platform="linear"
                          className="h-4 w-4 mr-2"
              ...
              <div className="rounded-xl bg-background border border-border overflow-hidden">
                <div className="relative p-3">
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <Switch
                      checked={respondToTrigger}
                      onCheckedChange={setRespondToTrigger}
                    />
                  </div>
                </div>
              </div>
                      ...
                      {allExecutions.length === 0 ? (
                        <div className="p-6 text-center">
                          <p className="text-sm text-muted-foreground">
                            No runs yet. This automation will appear here once
                            triggered.
                          </p>
                        </div>
                      ) : (
