...
- **`openspec/`** — OpenSpec 1.2.0 change proposals and 13 capability specs (91 requirements). Active changes: `upgrade-vite-8-build-stack` (15/50, Phase B blocked on electron-vite 6.0), `security-hardening-and-quality-remediation` (81/81 tasks — Phase A+B complete via PRs #17+#18, Phase C §6 CSP/CodeQL/Trivy/SSRF + §7 claude.ts decomposition (4 new modules: prompt-parser, session-manager, mcp-resolver, tool-executor; claude.ts 3309 → 2503 lines, −24%; chat handler residual tracked as P3 roadmap entry) + §8 all 11 tasks + §9 arch docs + §10 Phase D roadmap entries, all shipped 2026-04-12). Ready to `/opsx:verify` → `/opsx:archive`. See [`.claude/rules/openspec.md`](.claude/rules/openspec.md).
- **`tests/regression/`** — 20 bun:test files (19 regression guards + 1 frontmatter shim unit test) + service tests in `services/1code-api/tests/` (231 tests across 40 files total; 221 pass + 10 skipped integration tests behind `INTEGRATION_TEST=1` + docker-compose harness at `services/1code-api/tests/integration/`). See [`docs/conventions/regression-guards.md`](docs/conventions/regression-guards.md).
