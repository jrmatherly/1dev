...
- **IDE config:** `.vscode/settings.json` tracked in git — tsgo flag + SonarLint rule suppressions (53 rules disabled project-wide, covering TS/JS/CSS). Note: `sonarlint.rules` has `application` scope — VS Code ignores workspace settings; each dev must copy the block to User settings. See `docs/conventions/quality-gates.md` § "SonarLint IDE vs. `bun run lint`" for the full explanation.
