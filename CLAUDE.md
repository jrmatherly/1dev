...
- **TS baseline:** 26 pre-existing errors (reduced from 32 on 2026-04-11 incidentally during `add-1code-api-litellm-provisioning` work — the new `services/1code-api/` provisioning code is fully typed and adjacent Desktop-app files were auto-rechecked clean), enforced **both** locally (PostToolUse hook) **and in CI** (`ci.yml` reads `.claude/.tscheck-baseline` and fails if count exceeds it). Only new errors fail gates. See [`.claude/rules/tscheck-baseline.md`](.claude/rules/tscheck-baseline.md).
