...
- **`openspec/`** — OpenSpec 1.2.0 change proposals and 15 capability specs (109 requirements). Active changes: `add-dual-mode-llm-routing` (28/55, Groups 1-7 landed as commit `51318e1` — Entra decoupling + dual-mode routing; Groups 8-10 pending UI wizard + litellmModels router + docs), `remediate-dev-server-findings` (58/71, Groups 1-19 landed across commits `0f43165` + `3b37397` + `96af6c5` + `01d451e` — auth hardening, provider-aware aux-AI module at `src/main/lib/aux-ai.ts` replacing upstream apollosai.dev agent endpoints, signed-fetch upstream-disabled gate + 60s negative cache, 3 new regression guards, F11/F12 catalog entries; Group 18 manual smoke + Group 20 archive remain operator-driven), `improve-dev-launch-keychain-ux` (0/23, proposal scaffolded as commit `83d0d84` — ShipIT editor/terminal detection pattern for dev launch UX), `wire-login-button-to-msal` (45/57, MSAL sign-in flow end-to-end — awaiting manual smoke §11 + archive), `upgrade-vite-8-build-stack` (15/50, Phase B blocked on electron-vite 6.0). Recently archived: `security-hardening-and-quality-remediation` on 2026-04-13. See [`.claude/rules/openspec.md`](.claude/rules/openspec.md).
