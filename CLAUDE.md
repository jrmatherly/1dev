...
- **OpenSpec capability specs inventory**: `openspec/specs/` contains 4 promoted capabilities: `brand-identity` (Tier A/B/C brand taxonomy), `feature-flags` (feature flag infrastructure shape), `claude-code-auth-import` (local-only Claude Code auth paths), `documentation-site` (docs/ site contract, no-scratchpad-references rule, xyd build requirement). Use `## MODIFIED Requirements` against these baselines when changing their behavior.
...
- Regression guard count under `tests/regression/` — currently **9** (27 tests); grows whenever a new Phase 0 gate or brand guard lands
...
- Baseline spec count in `openspec/specs/` — currently **4** (`brand-identity`, `feature-flags`, `claude-code-auth-import`, `documentation-site`); grows when changes archive
