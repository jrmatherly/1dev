...
- **`openspec/`** — OpenSpec 1.2.0 change proposals and 15 capability specs (109 requirements). Active changes: `wire-login-button-to-msal` (45/57, MSAL sign-in flow end-to-end — awaiting manual smoke §11 + archive), `upgrade-vite-8-build-stack` (15/50, Phase B blocked on electron-vite 6.0). Recently archived: `security-hardening-and-quality-remediation` on 2026-04-13 (81/81 tasks; +18 requirements across 5 specs — 3 existing baselines expanded + 2 new baselines `electron-security-hardening` and `sqlite-performance`). See [`.claude/rules/openspec.md`](.claude/rules/openspec.md).
- **`tests/regression/`** — 21 bun:test files (20 regression guards + 1 frontmatter shim unit test) + service tests in `services/1code-api/tests/` (242 tests across 41 files total; 232 pass + 10 skipped integration tests behind `INTEGRATION_TEST=1` + docker-compose harness at `services/1code-api/tests/integration/`). See [`docs/conventions/regression-guards.md`](docs/conventions/regression-guards.md).
