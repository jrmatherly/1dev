...
- **`openspec/`** — OpenSpec 1.2.0 change proposals and 12 capability specs (85 requirements). Active changes: `replace-gray-matter-with-front-matter` (0/67), `upgrade-vite-8-build-stack` (15/50, Phase B blocked on electron-vite 6.0). See [`.claude/rules/openspec.md`](.claude/rules/openspec.md).
- **`tests/regression/`** — 15 bun:test regression guards + service tests in `services/1code-api/tests/` (199 tests across 35 files total; 189 pass + 10 skipped integration tests behind `INTEGRATION_TEST=1` + docker-compose harness at `services/1code-api/tests/integration/`). See [`docs/conventions/regression-guards.md`](docs/conventions/regression-guards.md).
