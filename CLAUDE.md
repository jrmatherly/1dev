...
- **`openspec/`** — OpenSpec 1.2.0 change proposals and 15 capability specs (109 requirements). Active change: `upgrade-vite-8-build-stack` (15/50, Phase B blocked on electron-vite 6.0). Recently archived: `security-hardening-and-quality-remediation` on 2026-04-13 (81/81 tasks; +18 requirements across 5 specs — 3 existing baselines expanded + 2 new baselines `electron-security-hardening` and `sqlite-performance`). See [`.claude/rules/openspec.md`](.claude/rules/openspec.md).
