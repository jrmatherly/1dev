...
- **OpenSpec capability specs inventory**: `openspec/specs/` contains 8 promoted capabilities: `brand-identity` (Tier A/B/C brand taxonomy), `feature-flags` (feature flag infrastructure shape), `claude-code-auth-import` (local-only Claude Code auth paths), `documentation-site` (docs/ site contract, no-scratchpad-references rule, xyd build requirement), `credential-storage` (3-tier credential encryption policy, enterprise hard-refusal flag), `renderer-data-access` (camelCase timestamp contract, F1/F2 boundary preservation, mock-api fossil guard), `enterprise-auth` (MSAL Node integration contract, tier-aware cache, isolation boundary), `electron-runtime` (supported Electron version + native module ABI + build toolchain). Use `## MODIFIED Requirements` against these baselines when changing their behavior.
...
- Baseline spec count in `openspec/specs/` — currently **8** (`brand-identity`, `feature-flags`, `claude-code-auth-import`, `documentation-site`, `credential-storage`, `renderer-data-access`, `enterprise-auth`, `electron-runtime`); grows when changes archive
