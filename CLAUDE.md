...
- **`openspec/`** — OpenSpec 1.2.0 change proposals and 15 capability specs (109 requirements). Active changes: `add-dual-mode-llm-routing` (28/55, Groups 1-7 landed as commit `51318e1` — Entra decoupling + dual-mode routing; Groups 8-10 pending UI wizard + litellmModels router + docs), `remediate-dev-server-findings` (scaffolded, unimplemented — revised per parallel code review to absorb 5 Critical + 13 Important findings from dual-mode + dev-server smoke), `wire-login-button-to-msal` (45/57, MSAL sign-in flow end-to-end — awaiting manual smoke §11 + archive), `upgrade-vite-8-build-stack` (15/50, Phase B blocked on electron-vite 6.0). Recently archived: `security-hardening-and-quality-remediation` on 2026-04-13. See [`.claude/rules/openspec.md`](.claude/rules/openspec.md).
- **`tests/regression/`** — 25 bun:test files (24 regression guards + 1 frontmatter shim unit test) + service tests in `services/1code-api/tests/` (242 tests across 41 files total; 232 pass + 10 skipped integration tests behind `INTEGRATION_TEST=1` + docker-compose harness at `services/1code-api/tests/integration/`). See [`docs/conventions/regression-guards.md`](docs/conventions/regression-guards.md).
